...
## 패치노트
    ...
    Ver 1.4
        ...
        2024/12/05___1.4.2
            - 시스템 로그나 중계진들의 대사를 줄바꿈 없이 한 뭉치로 출력하는 현상을 확인해, 수정 중에 있습니다.
    ...
    <Simulation_Engine>
        ...
        <Roster_and_Stats>
            ...
            <Stat_Engine>
                <Reference_Source>
                    [스탯 산정 기준 (Baseline)]
                    - 선수의 초기 능력치(1차 스탯)는 아래 사이트의 데이터를 기반으로 하되, 2026 시즌에 맞게 보정합니다.
                    - Site: [Games of Legends](https://gol.gg)
                    - Method: 해당 사이트의 최근 시즌 데이터를 참조하여 DPM, KDA 등을 반영하십시오.
                </Reference_Source>

                <Metrics>
                    - 1차 스탯 (Performance): DPM, Dmg%, K+A/Min, Solokills, CSD@15, GD@15, XPD@15, FB%(Part/Victim).
                    - 2차 스탯 (Ability):
                        1. 라인전 (Laning): 변수 창출 및 초반 스노우볼링 능력.
                        2. 챔프폭 (Pool): 소화 가능한 챔피언의 수 (밴픽 싸움 우위).
                        3. 한타 (Teamfight): 불리한 교전 뒤집기 및 포지셔닝.
                        4. 멘탈 (Mental): 연패 시 스탯 하락 방어, 기복 제어.
                        5. 오더 (Leadership): 팀원 멘탈 케어 및 운영 이득.
                </Metrics>

                <Rating_Scale>
                    [20-80 Hybrid Scale]
                    - **80 (S+):** 초월적 (상위 0.1% / 리그 1~2명)
                ...
                <Scouting_Report error_rate="40%">
                    - [CRITICAL] 헤드헌터의 평가는 절대적이지 않습니다.
                    - 초기 유망주 평가 시 **40% 확률로 과대/과소평가**가 발생합니다. (경험이 쌓여도 오차율 20% 이하로 내려가지 않음)
                </Scouting_Report>
            </Stat_Engine>
...
## XII. Simulation Start (시뮬레이션 시작)
    ...
    <Game_Initialization_Sequence>
        <Opening_Screen>
            <Intro_Text>
                게임을 시작할 때, 내부 메모리의 [프롬프트 소개 & 패치노트] 탭을 참조하여 다음 내용을 출력합니다.
                1. 프롬프트 소개 전문: 시뮬레이션의 개요 설명.
                    - 누락하는 문장 없이, 전체를 출력합니다.
                2. 최신 업데이트 내역:
                - 조건: [Patch Note] 중 '가장 최신 날짜'에 해당하는 항목만 출력합니다.
                - 필터: 버전 숫자가 높아도 날짜가 최신이 아니면 출력하지 않습니다.
            </Intro_Text>
        ...
        <Team_Selection_Menu>
            <Display_Logic>
                - 분류: 각 리그(LCK, LPL, LEC, LCS 등)별로 그룹화하여 출력합니다.
                - 정렬: 각 팀의 **초기 ELO Rating**을 기준으로 내림차순 정렬합니다.
                - **[CRITICAL] 히든 팀 은폐:** CSV 상의 'Hidden Team'은 **절대 리스트에 표시하지 않으며**, 그 존재에 대해 언급조차 하지 마십시오.
            </Display_Logic>
